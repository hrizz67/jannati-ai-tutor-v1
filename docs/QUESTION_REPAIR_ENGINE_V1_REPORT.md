# Question Repair Engine v1 Report

## Summary

- Total repair suggestions: 2726
- P1 repair list: 2
- P2/P3 suggestions: 2724
- Estimated cleanup priority: MEDIUM

## Suggestions by Subject

| Subject | Suggestion Count |
| --- | ---: |
| Bahasa Melayu Tahun 2 | 653 |
| English Year 2 | 422 |
| Matematik Tahun 2 | 417 |
| Bahasa Arab Tahun 2 | 343 |
| Pendidikan Jasmani Tahun 2 | 328 |
| Pendidikan Kesihatan Tahun 2 | 314 |
| Pendidikan Islam Tahun 2 | 140 |
| Sains Tahun 2 | 109 |

## Suggestions by Issue Type

| Issue Type | Suggestion Count |
| --- | ---: |
| same_answer_pattern_repeated | 2290 |
| multiple_possible_answers | 281 |
| missing_arabic_text | 90 |
| ambiguous_operation | 39 |
| identical_question_text | 15 |
| too_long | 9 |
| missing_instruction | 2 |

## Top P1 Repair List

- [Pendidikan Jasmani Tahun 2 / Pergerakan Asas] PJ-PERGERAKAN_ASAS-023 - missing_instruction: Question text is incomplete or missing context. -> Add a complete sentence and clear instruction.
- [Pendidikan Jasmani Tahun 2 / Pergerakan Asas] PJ-PERGERAKAN_ASAS-045 - missing_instruction: Question text is incomplete or missing context. -> Add a complete sentence and clear instruction.

## Top Recurring Problems

- Bahasa Melayu Tahun 2 / same_answer_pattern_repeated (P3): 653
- English Year 2 / same_answer_pattern_repeated (P3): 403
- Matematik Tahun 2 / same_answer_pattern_repeated (P3): 373
- Pendidikan Jasmani Tahun 2 / same_answer_pattern_repeated (P3): 326
- Pendidikan Kesihatan Tahun 2 / same_answer_pattern_repeated (P3): 314
- Bahasa Arab Tahun 2 / multiple_possible_answers (P2): 231
- Sains Tahun 2 / same_answer_pattern_repeated (P3): 109
- Bahasa Arab Tahun 2 / missing_arabic_text (P2): 90
- Pendidikan Islam Tahun 2 / same_answer_pattern_repeated (P3): 90
- Pendidikan Islam Tahun 2 / multiple_possible_answers (P2): 50
- Matematik Tahun 2 / ambiguous_operation (P2): 39
- Bahasa Arab Tahun 2 / same_answer_pattern_repeated (P3): 22
- English Year 2 / identical_question_text (P3): 10
- English Year 2 / too_long (P3): 9
- Matematik Tahun 2 / identical_question_text (P3): 5
- Pendidikan Jasmani Tahun 2 / missing_instruction (P1): 2

## Repair Readiness

READY FOR REPAIR PLANNING
