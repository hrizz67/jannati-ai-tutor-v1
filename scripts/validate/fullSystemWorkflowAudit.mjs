...
const checks = {
  ...
  githubPagesBase: /base:|homepage/.test(read('vite.config.mjs')) || Boolean(packageJson.homepage),
...
const report = { status: failures.length ? 'FAIL' : 'PASS', routes: [...new Set(routes)], subjectCount: subjectList.length, checks, failures, knownWarnings: ['physical browser/device execution still requires recorded hardware evidence'] };
