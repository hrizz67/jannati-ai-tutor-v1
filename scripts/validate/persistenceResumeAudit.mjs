...
const checks = {
  ...
  resumePersistence: /RESUME_KEY|persistResumeData|loadResume/i.test(resumeSources),
  legacyMigration: /LEGACY_|legacy|normalize/i.test(resumeSources) && files.some(({ text }) => /legacy|migrat|normalize/i.test(text)),
  parseGuard: files.every(({ text }) => /try\s*\{|JSON\.parse/.test(text)),
  clearPath: /clearResumeData|removeItem|resetProfile/.test(app),
  subjectScopedResume: /subjectId|topicId|questionIndex/.test(resumeSources) && /RESUME_SLOTS_KEY/.test(resumeStorage),
