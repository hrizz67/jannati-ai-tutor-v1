...
function main() {
  ...
  assert(/<BetaChrome recoveryMessages=\{recoveryMessages\} modalOpen=\{modalOpen\}[^>]*>/.test(appText), 'App should pass modalOpen into BetaChrome.', issues);
  ...
  assert(/<BetaChrome recoveryMessages=\{recoveryMessages\} modalOpen=\{modalOpen\}[^>]*><main className="app"><EmptyState title="Paparan tidak dijumpai\."/.test(appText), 'Fallback screen must also receive modalOpen.', issues);
