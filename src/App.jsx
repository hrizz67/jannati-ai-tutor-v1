  ...
  return <>
    ...
    {open && <div className="beta-feedback-overlay" role="dialog" aria-modal="true" aria-label="Maklum balas beta">
      <section className="beta-feedback-panel">
        <div className="beta-feedback-head"><div><p className="eyebrow">Beta Tertutup</p><h2>Maklum Balas</h2></div><button className="ghost" onClick={() => setOpen(false)}>Tutup</button></div>
...
function Dashboard({ profile, subjectList, allSubjects, selectedSubject, selectedSubjectId, totalQuestions, resume, dailyChallenge, onSelectSubject, onStartTopic, onStartAdaptiveLesson, onStartBacaan, onStartMendengar, onStartBertutur, onStartMenulis, onOpenParent, onOpenUasa, onOpenAi, onReset, onResume, onRestartResume, onCompleteDaily, onToggleFavourite }) {
  ...
  return <main className="dashboard-shell"><aside className="sidebar"><div className="brand"><div className="bot medium">🤖</div><div><h2>Jannati</h2><p>Tutor AI Ringan</p></div></div><button className="nav active">🏠 Papan Utama</button><button className="nav" onClick={onOpenAi}>🤖 Tutor AI</button><button className="nav" onClick={onOpenUasa}>🏆 UASA</button><button className="nav" onClick={onOpenParent}>👨‍👩‍👧 Ibu Bapa</button><div className="sidebar-note"><b>⚡ Data Ringan</b><p>Data dimuat ikut subjek supaya lebih ringan.</p></div></aside><section className="dashboard-main">
...
function AiTutorChat({ profile, selectedSubject, onTutup }) {
  const [messages, setMessages] = useState([{ role: 'ai', text: `Assalamualaikum ${profile.name || 'Anak'} 😊 Saya Tutor AI.` }]);
...
function ParentDashboard({ profile, allSubjects, onBack }) {
  ...
  return <main className="app parent-page">
    ...
    <section className="card parent-ai-card"><p className="eyebrow">Cadangan AI untuk Ibu Bapa</p><h2>🤖 Cadangan Ibu Bapa</h2><p>{recommendation}</p><div className="recommend-meta"><span>XP {profile.xp || 0}</span><span>Syiling {profile.coins || 0}</span><span>Hari berturut {profile.streak || 0}</span></div></section>
...
const speakingPrompts = [
  {
    ...
    prompts: {
      ...
      repeat: { label: 'Ulang ayat', text: 'Saya suka belajar bersama Jannati Tutor AI.', keywords: ['saya', 'suka', 'belajar', 'jannati'] }
    }
  },
  {
    id: 'english',
