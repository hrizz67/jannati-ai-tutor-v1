...
function Dashboard({ profile, subjectList, allSubjects, selectedSubject, selectedSubjectId, totalQuestions, resume, dailyChallenge, adaptivePracticePreview, adaptivePracticeCount, onAdaptivePracticeCountChange, onSelectSubject, onStartTopic, onStartAdaptiveLesson, onStartAdaptivePractice, onStartBacaan, onStartMendengar, onStartBertutur, onStartMenulis, onOpenParent, onOpenUasa, onOpenAi, onReset, onExportBetaReport, onResume, onRestartResume, onCompleteDaily, onToggleFavourite }) {
  ...
  const todayRevision = useMemo(() => getTodayRevision(adaptiveProfile), [adaptiveProfile]);
  const reviewQueue = useMemo(() => getReviewQueue(adaptiveProfile), [adaptiveProfile]);
  const difficultyPlan = useMemo(() => buildDifficultyPlan(adaptiveProfile), [adaptiveProfile]);
  const mixedRevisionSession = useMemo(() => buildMixedRevisionSession(adaptiveProfile, allSubjects, { questionCount: adaptivePracticeCount }), [adaptiveProfile, allSubjects, adaptivePracticeCount]);
  const revisionDifficulty = useMemo(() => getRecommendedDifficulty({
    mastery: todayRevision.priorityTopics?.[0]?.mastery ?? adaptiveProfile.mastery ?? 0,
    confidence: todayRevision.priorityTopics?.[0]?.confidence ?? adaptiveProfile.confidence ?? 0,
    total: adaptiveProfile.totalQuestions || 0,
    wrong: adaptiveProfile.correctQuestions ? Math.max(0, adaptiveProfile.totalQuestions - adaptiveProfile.correctQuestions) : 0,
    correct: adaptiveProfile.correctQuestions || 0,
    lastPlayed: adaptiveProfile.lastStudyDate || null,
    recentAnswers: adaptiveProfile.learningHistory || []
  }), [adaptiveProfile, todayRevision]);
  ...
  return <main className="dashboard-shell"><aside className="sidebar"><div className="brand"><BrandLogo iconOnly /><div><h2>Jannati</h2><p>AI Tutor Rasmi</p></div></div><button className="nav active">🏠 Papan Utama</button><button className="nav" onClick={onOpenAi}>🤖 Tutor AI</button><button className="nav" onClick={onOpenUasa}>🏆 UASA</button><button className="nav" onClick={onOpenParent}>👨‍👩‍👧 Ibu Bapa</button><div className="sidebar-note"><b>⚡ Data Ringan</b><p>Data dimuat ikut subjek supaya lebih ringan.</p></div></aside><section className="dashboard-main">    <DashboardHeader profile={profile} level={level} levelProgress={levelProgress} />    <section className="profile hero-card"><MascotCard character={dashboardCharacter} mood="happy" size="md" animation="gentle" message={`Assalamualaikum, ${profile.name || 'Anak'}. Hari ini kita akan belajar ${welcomeTopic}. Jom mulakan!`} /><div className="avatar-large">{profile.avatar || '\u{1F466}'}</div><div><p className="eyebrow">Edisi Data Ringan</p><h1>Assalamualaikum, {profile.name || 'Anak'}</h1><p>Hari ini kita akan belajar <b>{welcomeTopic}</b>. Jom mulakan!</p><div className="level-line"><span>Tahap {level}</span><div className="progress-wrap"><div className="progress" style={{ width: `${levelProgress}%` }} /></div><span>{levelProgress}/100 XP</span></div></div></section>
    ...
    <section className="card revision-today-card"><p className="eyebrow">Hari Ini</p><h2>Hari Ini</h2>{todayRevision.totalQuestions > 0 ? <><p>{todayRevision.summary}</p><div className="mastery-summary-grid"><div><b>{todayRevision.totalQuestions}</b><span>Today's Revision</span></div><div><b>{todayRevision.estimatedMinutes}</b><span>Estimated Time</span></div><div><b>{todayRevision.subjects.length}</b><span>Subjects</span></div><div><b>{todayRevision.priorityTopics.length}</b><span>Priority Topics</span></div><div><b>{revisionDifficulty.recommendedDifficulty}</b><span>Tahap Cadangan</span></div><div><b className={`difficulty-badge difficulty-${revisionDifficulty.recommendedDifficulty}`}>{revisionDifficulty.recommendedDifficulty}</b><span>Difficulty</span></div></div><div className="timeline"><div className="timeline-item"><span>Reason</span><b>{revisionDifficulty.reason}</b><em>Score {revisionDifficulty.score}%</em><p>{revisionDifficulty.distribution.mudah}% mudah • {revisionDifficulty.distribution.sederhana}% sederhana • {revisionDifficulty.distribution.sukar}% sukar</p></div></div><div className="subject-report-grid">{todayRevision.subjects.map(subject => <div className="report-box" key={`revision-${subject.subjectId}`}><h3>{subject.subjectId}</h3><b>{subject.questions}</b><div className="mini-progress"><div style={{ width: `${Math.min(100, Math.round((subject.questions / Math.max(1, todayRevision.totalQuestions)) * 100))}%` }} /></div><span>{subject.topics.length} topik • {subject.estimatedMinutes} min</span></div>)}</div><button type="button" className="full" onClick={() => onStartAdaptivePractice(todayRevision.totalQuestions || adaptivePracticeCount)}>Mulakan Ulang Kaji</button></> : <EmptyState title="Belum ada data pembelajaran." message="Mulakan beberapa latihan dahulu untuk menjana pelan ulang kaji." actionLabel="Mulakan Ulang Kaji" onAction={() => onStartAdaptivePractice(adaptivePracticeCount)} />}</section>
    <section className="card revision-queue-card"><p className="eyebrow">Jadual Ulang Kaji</p><h2>Jadual Ulang Kaji</h2>{reviewQueue.dueTopics.length || reviewQueue.upcomingTopics.length || reviewQueue.overdueTopics.length ? <><div className="mastery-summary-grid"><div><b>{reviewQueue.dueTopics.length}</b><span>Perlu Hari Ini</span></div><div><b>{reviewQueue.upcomingTopics.length}</b><span>Akan Datang</span></div><div><b>{reviewQueue.overdueTopics.length}</b><span>Tertunggak</span></div><div><b>{reviewQueue.today}</b><span>Tarikh</span></div></div><div className="parent-two-col"><section className="card"><p className="eyebrow">Perlu Hari Ini</p><div className="timeline">{reviewQueue.dueTopics.length ? reviewQueue.dueTopics.slice(0, 5).map(topic => <div className="timeline-item" key={`due-${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Priority {topic.priority} • {topic.nextReview}</em><p>Penguasaan {topic.mastery}% • Keyakinan Data {topic.confidence}%</p></div>) : <EmptyState title="Tiada ulang kaji diperlukan hari ini." message="Semua topik telah dijadualkan dengan baik." />}</div></section><section className="card"><p className="eyebrow">Akan Datang</p><div className="timeline">{reviewQueue.upcomingTopics.length ? reviewQueue.upcomingTopics.slice(0, 5).map(topic => <div className="timeline-item" key={`upcoming-${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Priority {topic.priority} • {topic.nextReview}</em></div>) : <EmptyState title="Tiada topik akan datang." message="Topik baharu akan muncul selepas lebih banyak latihan." />}</div></section></div><section className="card"><p className="eyebrow">Tertunggak</p><div className="timeline">{reviewQueue.overdueTopics.length ? reviewQueue.overdueTopics.slice(0, 5).map(topic => <div className="timeline-item" key={`overdue-${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Priority {topic.priority} • {topic.nextReview}</em></div>) : <EmptyState title="Tiada topik tertunggak." message="Teruskan rutin yang stabil." />}</div></section><button type="button" className="full" onClick={() => onStartAdaptivePractice(adaptivePracticeCount)}>Mula Latihan AI</button></> : <EmptyState title="Tiada ulang kaji diperlukan hari ini." message="Mulakan beberapa latihan dahulu untuk menjana jadual ulang kaji." actionLabel="Mulakan Ulang Kaji" onAction={() => onStartAdaptivePractice(adaptivePracticeCount)} />}</section>
    <section className="card revision-mixed-card"><p className="eyebrow">Sesi Ulang Kaji AI</p><h2>Sesi Ulang Kaji AI</h2><div className="mastery-summary-grid"><div><b>{mixedRevisionSession.totalQuestions}</b><span>Jumlah Soalan</span></div><div><b>{mixedRevisionSession.subjects.length}</b><span>Subjek</span></div><div><b>{difficultyPlan.distribution.mudah}% / {difficultyPlan.distribution.sederhana}% / {difficultyPlan.distribution.sukar}%</b><span>Tahap</span></div><div><b>{mixedRevisionSession.estimatedMinutes}</b><span>Masa</span></div></div><div className="subject-report-grid">{mixedRevisionSession.subjects.map(subject => <div className="report-box" key={`mixed-${subject.subjectId}`}><h3>{subject.subjectId}</h3><b>{subject.questions}</b><div className="mini-progress"><div style={{ width: `${Math.min(100, Math.round((subject.questions / Math.max(1, mixedRevisionSession.totalQuestions)) * 100))}%` }} /></div><span>{subject.topics.length} topik</span></div>)}</div><button type="button" className="full" onClick={() => onStartAdaptivePractice(mixedRevisionSession.totalQuestions || adaptivePracticeCount)}>Mula Sesi AI</button></section>
...
function ParentDashboard({ profile, allSubjects, adaptivePracticeCount, onStartAdaptivePractice, onBack }) {
  ...
  const parentReport = useMemo(() => generateParentReport(adaptiveProfile, { questionCount: adaptivePracticeCount, mode: 'weekly' }), [adaptiveProfile, adaptivePracticeCount]);
  ...
  const reportHasData = Boolean(parentReport?.hasData);
  const reportWeakTopic = parentReport?.improvements?.[0] || 'Belum cukup data untuk analisis subjek.';
  const reportAdvice = parentReport?.advice || [];

  return <main className="app parent-page">
    <div className="topbar"><button className="ghost" onClick={onBack}>← Papan Utama</button><button onClick={printParentReport}>🖨️ Cetak Laporan</button></div>
    <div className="parent-print-report">
    <section className="card parent-hero"><BrandLogo iconOnly /><div><p className="eyebrow">Laporan Pembelajaran Murid</p><h1>Laporan Pembelajaran {profile.name || 'Anak'}</h1><p>Tarikh Laporan: <b>{new Date().toISOString().slice(0, 10)}</b></p></div></section>
    ...
    <section className="card"><p className="eyebrow">Laporan AI</p><h2>Laporan AI</h2>{reportHasData ? <><div className="parent-summary-grid"><div className="parent-metric"><span>Ringkasan Mingguan</span><b>{parentReport.summary}</b><small>{parentReport.generatedAt.slice(0, 10)}</small></div><div className="parent-metric"><span>Pencapaian</span><b>{parentReport.achievements.length}</b><small>{parentReport.achievements[0] || 'Belum cukup data.'}</small></div><div className="parent-metric"><span>Perlu Diperbaiki</span><b>{parentReport.improvements.length}</b><small>{reportWeakTopic}</small></div><div className="parent-metric"><span>Cadangan AI</span><b>{parentReport.estimatedStudyTime} min</b><small>{reportAdvice[0] || 'Cadangan akan muncul apabila data mencukupi.'}</small></div></div><div className="timeline">{parentReport.achievements.slice(0, 3).map((item, index) => <div className="timeline-item" key={`achievement-${index}`}><span>Pencapaian</span><b>{item}</b><em>{parentReport.encouragement}</em></div>)}</div><div className="timeline">{parentReport.improvements.slice(0, 3).map((item, index) => <div className="timeline-item" key={`improvement-${index}`}><span>Perlu Diperbaiki</span><b>{item}</b><em>{parentReport.nextGoal}</em></div>)}</div><div className="timeline">{reportAdvice.slice(0, 3).map((item, index) => <div className="timeline-item" key={`advice-${index}`}><span>Cadangan AI</span><b>{item}</b><em>{parentReport.estimatedStudyTime} min</em></div>)}</div><div className="timeline"><div className="timeline-item"><span>Galakan</span><b>{parentReport.encouragement}</b><em>Motivasi Harian</em></div><div className="timeline-item"><span>Matlamat Seterusnya</span><b>{parentReport.nextGoal}</b><em>Matlamat Seterusnya</em></div></div></> : <EmptyState title="Belum ada rekod pembelajaran." message="AI memerlukan lebih banyak data pembelajaran sebelum laporan boleh dijana." actionLabel="Mula Latihan AI" onAction={() => onStartAdaptivePractice(adaptivePracticeCount)} />}</section>
    </div>
