...
function Dashboard({ profile, subjectList, selectedSubject, selectedSubjectId, totalQuestions, resume, dailyChallenge, onSelectSubject, onStartTopic, onStartReading, onOpenParent, onOpenUasa, onOpenAi, onReset, onResume, onRestartResume, onCompleteDaily, onToggleFavourite }) {
  ...
  return <main className="dashboard-shell"><aside className="sidebar"><div className="brand"><div className="bot medium">🤖</div><div><h2>Jannati</h2><p>AI Tutor Split</p></div></div><button className="nav active">🏠 Dashboard</button><button className="nav" onClick={onOpenAi}>🤖 AI Tutor</button><button className="nav" onClick={onOpenUasa}>🏆 UASA</button><button className="nav" onClick={onOpenParent}>👨‍👩‍👧 Parent</button><div className="sidebar-note"><b>⚡ Split Data</b><p>Data dimuat ikut subjek supaya lebih ringan.</p></div></aside><section className="dashboard-main">
    ...
    <section className="card learning-path-card"><p className="eyebrow">Learning Path</p><h2>{selectedSubject.icon} {selectedSubject.title}</h2><p>{topics.length} topik • {totalQuestions} soalan</p><div className="learning-path">{topics.map((topic, index) => { const best = profile.progress?.[progressKey(selectedSubject.id, topic.id)]?.best || 0; const unlocked = isTopicUnlocked(profile, selectedSubject, index); const favId = `${selectedSubject.id}_${topic.id}`; const isFav = (profile.favourites || []).some(f => f.id === favId); return <div className="path-row" key={topic.id}><button className={`path-node ${best >= 80 ? 'path-done' : ''} ${!unlocked ? 'path-locked' : ''}`} onClick={() => unlocked ? onStartTopic(topic) : alert('Dapatkan 80% pada topik sebelumnya.')}><span className="path-icon">{unlocked ? (best >= 80 ? '🏅' : index + 1) : '🔒'}</span><div><b>{topic.title}</b><small>{topic.questions.length} soalan • {best}% • {getStars(best)}</small><div className="mini-progress"><div style={{ width: `${best}%` }} /></div></div></button><button className="fav-btn" onClick={() => onToggleFavourite(selectedSubject.id, topic.id, topic.title)}>{isFav ? '❤️ Favourite' : '🤍 Favourite'}</button>{index < topics.length - 1 && <div className="path-line">↓</div>}</div> })}<div className="path-trophy">🏆 Tamat {selectedSubject.short}</div></div></section>
  </section></main>;
}
