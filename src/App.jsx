...
import { appendCreditedQuizAttempt, canRetryQuizAnswer, getBestCreditedQuizOutcome, isQuizAnswerChecked, reconcileQuizSessionCredits, summarizeCreditedQuizOutcomes } from './utils/quizSessionOutcome.js';
...
function writeChildProfiles(profiles) {
  const nextValue = JSON.stringify((profiles || []).map(normalizeChildProfile));
  if (localStorage.getItem(CHILD_PROFILES_KEY) !== nextValue) {
    localStorage.setItem(CHILD_PROFILES_KEY, nextValue);
  }
}
...
function writeArchivedChildren(records = {}) {
  ...
  if (localStorage.getItem(ARCHIVED_CHILDREN_KEY) !== nextValue) {
    localStorage.setItem(ARCHIVED_CHILDREN_KEY, nextValue);
  }
}
...
function quarantineChildScopeFailure(childId, snapshot, issues = []) {
  try {
    ...
    persistBoundedChildSnapshot(localStorage, backupKey, {
      version: 1,
  ...
  try {
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      if (Object.keys(cleaned).length !== Object.keys(snapshot).length) {
        ...
        if (originalSnapshot) persistBoundedChildSnapshot(localStorage, originalKey, originalSnapshot, { removeExistingOnLimit: true });
        persistBoundedChildSnapshot(localStorage, key, cleaned, { removeExistingOnLimit: true });
      }
    }
  } catch {
...
function backupChildBeforeDeletion(profile = {}) {
  ...
  try {
    ...
    const persisted = persistBoundedChildSnapshot(localStorage, backupKey, {
      version: 1,
...
function captureAccountSnapshot(accountId) {
  if (!accountId) return false;
  const result = captureCompactAccountSnapshot(localStorage, accountId, readLocalAccountData());
...
async function restoreCloudLearningSnapshotResult(cloudData = {}, preferredChildId = '') {
  ...
  const result = hydrateCloudLearningState(localStorage, cloudData, {
    ...
    preferredChildId,
    currentActiveChildId: readActiveChildId()
  });
  if (import.meta.env.DEV && result.reason !== 'ok') {
    console.info('Cloud restore diagnostic', {
      accountId: accountScopeId,
      hydrated: Boolean(result.hydrated),
      activeStatePersisted: Boolean(result.activeStatePersisted),
      snapshotPersisted: Boolean(result.snapshotPersisted),
      reason: result.reason,
      errorName: result.errorName || '',
      failedKey: result.failedKey || '',
      orphanSnapshotCount: result.orphanSnapshotKeys?.length || 0
    });
  }
  return { ...result, childId: result.activeChildId || '' };
}
  ...
  try {
    ...
    Object.entries(payload).forEach(([key, raw]) => {
      if (key.startsWith(CHILD_MERGED_BACKUP_PREFIX) && typeof raw === 'string') {
        persistBoundedChildSnapshot(localStorage, key, raw);
        return;
      }
      ...
      if (deletedChildren[childId]) {
        ...
        persistBoundedChildSnapshot(localStorage, key, raw, { removeExistingOnLimit: true });
      }
    });
...
function activateAccountStorage(accountId) {
  ...
  try {
    ...
    const boundedSnapshot = existingSnapshot
      ? buildCompactAccountSnapshot(existingSnapshot, nextId).snapshot
      : null;
    if (boundedSnapshot && !restoreAccountSnapshot(boundedSnapshot, nextId)) clearAccountData();
...
export default function App() {
  ...
  function closeAdminPremium() {
    ...
    if (accountUser?.id && cloudHydratedAccountId !== accountUser.id) {
  ...
  function applyCloudRestoreResult(restoreResult, user, preserveQuizUi = false) {
    const profiles = Array.isArray(restoreResult?.profiles) ? restoreResult.profiles : [];
    const restoredChildId = String(restoreResult?.childId || '').trim();
    const activeChild = profiles.find(item => item.id === restoredChildId) || profiles[0] || null;
    setChildProfiles(profiles);
    setArchivedChildren(restoreResult?.archivedChildren || {});
    setActiveChildId(activeChild?.id || '');

    const blocked = profiles.length > 0 && !restoreResult?.activeStatePersisted;
    cloudWriteGuardRef.current = {
      accountId: String(user?.id || ''),
      blocked,
      serverProfileCount: profiles.length,
      reason: blocked ? String(restoreResult?.reason || 'cloud_hydration_incomplete') : ''
    };

    if (restoreResult?.activeStatePersisted) {
      repairImportedLearningProfile();
      return reloadCloudLearningState(activeChild?.id || '', preserveQuizUi);
    }

    const parseProjection = (key, fallback = {}) => {
      try {
        const parsed = JSON.parse(restoreResult?.projection?.[key] || 'null');
        return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : fallback;
      } catch {
        return fallback;
      }
    };
    const projectedProfile = parseProjection(PROFILE_KEY, {});
    const projectedCore = parseProjection('jannati_v152_student_core', {});
    const profileCandidate = projectedCore.profile && typeof projectedCore.profile === 'object'
      ? projectedCore.profile
      : projectedProfile;
    const identity = getLearningStorageScope({
      accountId: user?.id || getActiveStorageScopeId(),
      childId: activeChild?.id || '',
      profile: profileCandidate
    });
    setProfile(stampLearningIdentity(applyAuthoritativeProfileAccess({
      ...defaultProfile,
      ...profileCandidate,
      name: activeChild?.name || profileCandidate.name || 'Murid',
      year: activeChild?.year || profileCandidate.year || 'Tahun 2',
      avatar: activeChild?.avatar || profileCandidate.avatar,
      accountId: user?.id || profileCandidate.accountId || ''
    }), identity));
    setAdaptiveProfile(parseProjection('jannati.adaptive.studentProfile', {}));
    setGamificationProfile(parseProjection('jannati.gamification.profile', {}));
    setResume(null);
    if (!preserveQuizUi) {
      setFeedback(null);
      setAnswer('');
  ...
  function clearTransientChildState() {
    ...
    setAnswer('');
    setFeedback(null);
    const emptySession = { correct: 0, almost: 0, wrong: 0, xp: 0, coins: 0, percent: 0, stars: '☆☆☆', answers: [] };
  ...
  useEffect(() => {
    ...
    const hydrationStageByAccount = new Map();
    ...
    const openAccountDashboardFromDevice = user => {
      ...
      try {
        ...
        // Optional onboarding metadata must never block the local dashboard.
      }
    ...
    const syncAccount = async (user) => {
      ...
      const currentHash = typeof window === 'undefined' ? '' : String(window.location.hash || '').trim();
      ...
      if (loadStudentLearning && dashboardReadyAccountId !== user.id) {
        setCloudSyncInfo({ revision: 0, serverUpdatedAt: '' });
        lastCloudSignatureRef.current = '';
        try {
          ...
          hydrationStageByAccount.set(user.id, 'open-device-dashboard');
      ...
      const hydration = await settleAccountHydration({
        ...
        loadLearning: signal => loadStudentLearning
          ? loadCloudLearningDataResult(supabase, { signal })
          : Promise.resolve({
            data: {},
            revision: 0,
      ...
      if (!loadStudentLearning) {
        ...
        setShowOnboarding(false);
        setShowAccountLogin(false);
        hydrationStageByAccount.set(user.id, 'admin-complete');
        return;
      }
      ...
      if (!cloudResult.error && Object.keys(cloudLearningData).length && !hasPendingLocalData) {
        ...
        if (!restoreResult.ok) {
          ...
          setRecoveryMessages(prev => [
            ...prev,
            `Data cloud belum dapat dipulihkan pada peranti ini (kod ${restoreResult.reason}). Data semasa dikekalkan dan cubaan semula dijadualkan.`
          ]);
          scheduleHydrationRetry(user);
          return;
        }
      ...
      hydrationStageByAccount.set(user.id, 'load-active-profile');
      const accountDisplayName = [
        user.user_metadata?.display_name,
        data?.display_name,
        user.email?.split('@')[0]
      ...
      try {
        ...
        // Optional onboarding metadata must never fail account hydration.
      }
      setShowOnboarding(false);
      setShowAccountLogin(false);
      const nextAccess = entitlementResult.error || !entitlementResult.data
      ...
      hydrationStageByAccount.set(user.id, 'complete');
    };
    ...
    const requestAccountSync = user => {
      ...
      const task = syncAccount(user).catch(error => {
        ...
        if (user?.id) {
          ...
          setRecoveryMessages(prev => [
            ...prev,
            `Dashboard telah dibuka. Sambungan profil dan data cloud akan dicuba semula (kod ${stage}).`
          ]);
          scheduleHydrationRetry(user);
        }
      });
  ...
  async function loadLearningDataNow() {
    ...
    skipNextCloudSaveRef.current = true;
    const activeChildBeforeCloudRestore = activeChildId || readActiveChildId();
    const restoreResult = await restoreCloudLearningSnapshotResult(cloudData, activeChildBeforeCloudRestore);
    if (!restoreResult.ok) {
      lastCloudSignatureRef.current = '';
      setCloudSyncStatus('error');
      setRecoveryMessages(prev => [
        ...prev,
        `Data cloud tidak dapat dimuat dengan selamat (kod ${restoreResult.reason}). Data peranti dikekalkan.`
      ]);
      return;
    }
    applyCloudRestoreResult(
  ...
  async function startInteractivePracticeResume(targetResume, { restart = false } = {}) {
    ...
    startTopic(topic, subject, {
      ...
      ...(restart ? {} : {
        session: {
          ...(targetResume.session || {}),
          answers: Array.isArray(targetResume.answers) ? targetResume.answers : targetResume.session?.answers || []
        },
  ...
  function startTopic(topic, subject = selectedSubject, options = {}) {
    ...
    const startSession = reconcileQuizSessionCredits(
      { mode: resumeMode, correct: 0, almost: 0, wrong: 0, xp: 0, coins: 0, percent: 0, stars: '☆☆☆', answers: [], questions: [], diversityScore: diversity.score, diversityDebug: diversity.debug, ...(options.session || {}) },
  ...
  function checkAnswer() {
    ...
    const creditResult = appendCreditedQuizAttempt(liveSession, attempt, {
      questionIds: activeTopic.questions.map(item => item.id).filter(Boolean)
    });
    if (!creditResult.accepted) return;
    const { xp, coins } = creditResult.rewardDelta;
    const nextSession = {
      ...creditResult.session,
      questions: [...(creditResult.session.questions || []), question]
    };
    if (result.status === 'correct') beep('good');
  ...
  function tryAgainQuestion() {
    ...
    if (!question
      ...
      || getBestCreditedQuizOutcome(sessionRef.current?.answers, question.id) === 'correct') return;
    coachRequestRef.current = { ...coachRequestRef.current, open: false };
    questionStartedAtRef.current = Date.now();
    quizSubmitKeyRef.current = '';
    setAnswer('');
  ...
  function showQuestionHint() {
    ...
    if (!question
      || feedback?.status === 'correct'
      || getBestCreditedQuizOutcome(sessionRef.current?.answers, question.id) === 'correct') return;
    markQuestionSupport('usedHint');
    if (isQuizAnswerChecked(feedback)) quizSubmitKeyRef.current = '';
    const hint = buildChildSafeHint(question, activeSubject, [coachKnowledgeData?.hint, coachingDecision?.hint, teachingStrategy?.hint, question?.hint]);
    setFeedback({
      status: 'hint',
      title: 'Petunjuk',
      message: hint,
      teachingStyle: teachingStrategy?.teachingStyle || 'guided',
      explanationDepth: teachingStrategy?.explanationDepth || 1
    });
  }

  function nextQuestion() {
    if (!isQuizAnswerChecked(feedback)) return;
    coachRequestRef.current = { ...coachRequestRef.current, open: false };
  ...
  function finishTopic() {
    ...
    const questionIds = activeTopic.questions.map(item => item.id).filter(Boolean);
    const liveSession = sessionRef.current || session;
    ...
    const finishedSessionId = creditedSession.adaptiveSessionId || adaptiveSessionRef.current?.sessionId || '';
    const adaptiveSessionResult = recordSessionEnd(getAdaptiveProfile(learningIdentity), {
      sessionId: finishedSessionId,
      subjectId: activeSubject.id,
      topicId: activeTopic.id,
      questions: [...creditedSummary.bestByQuestion.keys()],
      plannedQuestionCount: total,
      correct: creditedSummary.correct,
      // Adaptive session accuracy is binary: "almost" remains not fully correct at session end.
      wrong: creditedSummary.almost + creditedSummary.wrong,
...
function Quiz({ subject, topic, questionIndex, answer, feedback, isBookmarked, coachDecision, teachingStrategy, personality, coachKnowledgeData, hasAccountSession, cloudSyncStatus, onAnswerChange, onCheckAnswer, onNextQuestion, onTryAgain, onExplain, onBack, onPetunjuk, onSpeak, onBookmark, onOpenAi }) {
  ...
  const answerChecked = isQuizAnswerChecked(feedback);
...
function getBertuturSpeechErrorMessage(errorCode = '') {
  switch (String(errorCode || '').toLowerCase()) {
    ...
    case 'service-not-allowed':
      return 'Mikrofon tidak dibenarkan. Benarkan akses mikrofon dalam tetapan peranti.';
    case 'no-speech':
    case 'no-result':
  ...
  return (
    <main className="app writing-coach-page">
      <div className="topbar">
        ...
        <span className="pill">Boleh digunakan tanpa internet</span>
      </div>
      ...
      <section className="card reading-hero">
        ...
        <div>
          <p className="eyebrow">Latihan Menulis</p>
          <h1>{set.title}</h1>
          <p>Tulis jawapan kamu. Janna akan membantu semak ejaan dan ayat.</p>
        </div>
      </section>
  ...
  return (
    <main className="app listening-lab-page">
      ...
      <section className="card reading-hero">
        ...
        <div>
          <p className="eyebrow">Latihan Mendengar</p>
          <h1>{item.title}</h1>
