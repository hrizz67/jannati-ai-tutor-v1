...
export default function AdminPremiumPage({ supabase, accountUser, onBack, onEntitlementChanged }) {
  ...
  return <main className="admin-premium-page">
    ...
    {selected ? <section className="card admin-premium-account-panel" aria-busy={loadingDetails}>
      ...
      <section className="admin-detail-section"><h3>Profil Anak</h3>{details?.children?.length ? <div className="admin-children-grid">{details.children.map(child => <article key={child.id}><div className="admin-child-heading"><strong>{child.name}</strong>{child.isActive ? <small className="admin-child-active">Aktif</small> : null}</div><span>{child.year}</span></article>)}</div> : <p className="admin-empty-copy">Tiada profil anak aktif.</p>}</section>
