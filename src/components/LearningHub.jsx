...
export default function LearningHub({ selectedSubject, allSubjects = [], profile = {}, onStartTopic, onMarkMaterial, onOpenAi, activeMode, onModeChange }) {
  ...
  return <section id="learning-hub-section" className="card learning-hub-card" aria-labelledby="learning-hub-title">
    <div className="learning-hub-heading">
      <div><p className="eyebrow">Pusat Belajar</p><h2 id="learning-hub-title">Nota & Panduan Buku Teks</h2><p>Belajar dahulu, kemudian ulang kaji dan cuba latihan.</p></div>
    ...
    <div className="learning-hub-tabs" role="tablist" aria-label="Jenis bahan pembelajaran">
      ...
      <button type="button" role="tab" aria-selected={visibleMode === 'buku'} className={visibleMode === 'buku' ? '' : 'secondary'} onClick={() => selectMode('buku')}><GameBadge className="learning-hub-tab-badge" src={bukuTeksBadge} /> Panduan buku teks</button>
    ...
    <div className="learning-hub-progress-summary" aria-label="Kemajuan bahan untuk subjek dipilih"><span>Nota dibaca <b>{noteCount}/{topics.length}</b></span><span>Panduan disemak <b>{textbookCount}/{topics.length}</b></span><small>Disimpan mengikut profil anak dan diselaraskan bersama data pembelajaran.</small></div>
