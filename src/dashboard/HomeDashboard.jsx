...
export default function HomeDashboard(props) {
  ...
  const studentData = {
    profile,
    adaptiveProfile: profile.adaptiveProfile || profile,
    adaptiveHasEvidence: (profile.totalQuestions || 0) > 0 || (profile.correctQuestions || 0) > 0 || (profile.studyMinutes || 0) > 0,
    overallAccuracy: profile.totalQuestions ? Math.round((profile.correctQuestions / profile.totalQuestions) * 100) : 0,
    adaptivePracticeCount,
    adaptivePracticePreview,
    adaptiveWeakTopics: [],
    ...
    streakBest: Number(profile.bestStreak || profile.streak || 0),
  ...
  const analyticsData = {
    ...
    statsCards: [
      ...
      { label: 'Soalan', value: profile.totalQuestions || 0, icon: '📝' },
      { label: 'Ketepatan', value: `${studentData.overallAccuracy}%`, icon: '🎯' },
      { label: 'Masa Belajar', value: formatStudyTime(profile.studyMinutes || 0), icon: '⏱️' },
