...
import { formatPriority, formatStatus, formatStudyMinutes, formatSubjectName, formatTopicName } from '../utils/displayFormatter';
...
export default function HomeDashboard(props) {
            ...
            : resume?.mode === 'writing' ? 'Menulis'
              : resume?.mode === 'adaptive-practice' ? 'Latihan AI'
                : resume?.mode === 'adaptive-lesson' ? 'Laluan Belajar'
                  : resume?.mode === 'quiz' ? 'Latihan' : 'Sambung Latihan'
  );
  ...
  const analyticsData = {
    ...
    statsCards: [
      ...
      { label: 'Ketepatan', value: `${studentData.overallAccuracy}%`, icon: '🎯' },
      { label: 'Masa Belajar', value: formatStudyMinutes(adaptiveStore.studyMinutes || 0), icon: '⏱️' },
