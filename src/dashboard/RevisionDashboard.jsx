      ...
      {todayRevision.totalQuestions > 0 ? (
        <>
          ...
          <div className="mastery-summary-grid">
            <div><b>{todayRevision.totalQuestions}</b><span>Today's Revision</span></div>
            <div><b>{todayRevision.estimatedMinutes}</b><span>Estimated Time</span></div>
            <div><b>{todayRevision.subjects.length}</b><span>Subjects</span></div>
            <div><b>{todayRevision.priorityTopics.length}</b><span>Priority Topics</span></div>
            <div><b>{revisionDifficulty.recommendedDifficulty}</b><span>Tahap Cadangan</span></div>
            <div><b className={`difficulty-badge difficulty-${revisionDifficulty.recommendedDifficulty}`}>{revisionDifficulty.recommendedDifficulty}</b><span>Difficulty</span></div>
          </div>
          <div className="timeline">
            <div className="timeline-item">
              <span>Reason</span>
              <b>{revisionDifficulty.reason}</b>
              <em>Score {revisionDifficulty.score}%</em>
              <p>{revisionDifficulty.distribution.mudah}% mudah • {revisionDifficulty.distribution.sederhana}% sederhana • {revisionDifficulty.distribution.sukar}% sukar</p>
            </div>
          </div>
        ...
        <>
          ...
          <div className="parent-two-col">
            <section className="card">
              ...
              <div className="timeline">
                {reviewQueue.dueTopics.length ? reviewQueue.dueTopics.slice(0, 5).map(topic => <div className="timeline-item" key={`due-${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Priority {topic.priority} • {topic.nextReview}</em><p>Penguasaan {topic.mastery}% • Keyakinan Data {topic.confidence}%</p></div>) : <EmptyState title="Tiada ulang kaji diperlukan hari ini." message="Semua topik telah dijadualkan dengan baik." />}
              </div>
            </section>
            <section className="card">
              <p className="eyebrow">Akan Datang</p>
              <div className="timeline">
                {reviewQueue.upcomingTopics.length ? reviewQueue.upcomingTopics.slice(0, 5).map(topic => <div className="timeline-item" key={`upcoming-${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Priority {topic.priority} • {topic.nextReview}</em></div>) : <EmptyState title="Tiada topik akan datang." message="Topik baharu akan muncul selepas lebih banyak latihan." />}
              </div>
            </section>
          </div>
          <section className="card">
            ...
            <div className="timeline">
              {reviewQueue.overdueTopics.length ? reviewQueue.overdueTopics.slice(0, 5).map(topic => <div className="timeline-item" key={`overdue-${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Priority {topic.priority} • {topic.nextReview}</em></div>) : <EmptyState title="Tiada topik tertunggak." message="Teruskan rutin yang stabil." />}
            </div>
          </section>
