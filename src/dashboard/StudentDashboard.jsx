...
export default function StudentDashboard({
  ...
  return <>
      ...
      {adaptiveHasEvidence ? (
        <>
          ...
          <div className="mastery-summary-grid">
            ...
            <div><b>{adaptiveProfile.studyMinutes || 0}</b><span>Masa Belajar</span></div>
          </div>
        </>
      ) : (
        <EmptyState title="Belum ada rekod pembelajaran." message="Mulakan dengan Latihan AI untuk membina profil adaptif." actionLabel="Mula Latihan AI" onAction={() => onStartAdaptivePractice(adaptivePracticeCount)} />
      )}
    </section>
    ...
    <section className="card">
      ...
      <div className="subject-report-grid">{(adaptiveRecommendation?.subjectRows || []).map(subject => <div className="report-box" key={`student-${subject.id}`}><h3>{subject.icon} {subject.title}</h3><b>{subject.accuracy}%</b><div className="mini-progress"><div style={{ width: `${subject.accuracy}%` }} /></div><span>{subject.total} soalan dicuba</span></div>)}</div>
    </section>

    <section className="card">
      <p className="eyebrow">Topik Perlu Diperbaiki</p>
      <h2>Topik Perlu Diperbaiki</h2>
      {topWeak.length ? <div className="timeline">{topWeak.map(topic => { const explanation = explainWeakness(topic); return <div className="timeline-item" key={`${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>{topic.status} • Penguasaan {topic.mastery}% • Keyakinan Data {topic.confidence}%</em><p>{explanation.message}</p></div>; })}</div> : <EmptyState title="Belum cukup data." message="Mula beberapa latihan untuk melihat topik yang perlu diperbaiki." />}
    </section>

    <section className="card">
      <p className="eyebrow">Topik Dikuasai</p>
      <h2>Topik Dikuasai</h2>
      {topStrong.length ? <div className="timeline">{topStrong.map(topic => <div className="timeline-item" key={`${topic.subjectId}-${topic.topicId}`}><span>{topic.subjectId}</span><b>{topic.topicId}</b><em>Penguasaan {topic.mastery}% • Keyakinan Data {topic.confidence}%</em></div>)}</div> : <EmptyState title="Belum cukup data." message="Topik dikuasai akan muncul selepas beberapa sesi." />}
    </section>

    <section className="card adaptive-recommendation-card">
      <p className="eyebrow">Cadangan Hari Ini</p>
      <h2>Cadangan Hari Ini</h2>
      <p>{adaptiveRecommendationFocus ? `Fokus utama: ${adaptiveRecommendationFocus.subjectId} • ${adaptiveRecommendationFocus.topicId}` : 'Fokus utama belum tersedia.'}</p>
      <div className="mastery-summary-grid">
